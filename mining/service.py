...
class MiningService(GenericService):
    ...
    def update_block(self):
        '''Connect this RPC call to 'Dashcoind -blocknotify' for
    ...
    @admin
    def add_dashcoind(self, *args):
        ''' Function to add a Dashcoind instance live '''
        if len(args) != 4:
        ...
        log.info("New Dashcoind connection added %s:%s" % (args[0], args[1]))
        return True
    ...
    def submit(self, worker_name, job_id, extranonce2, ntime, nonce):
        ...
        if on_submit is not None:
            # Pool performs submitblock() to Dashcoind. Let's hook
    ...
    submit.params = [('worker_name', 'string', 'Name of the worker, usually in the form of user_login.worker_id.'),
                     ('job_id', 'string',
                      'ID of job (received by mining.notify) which the current solution is based on.'),
                     ('extranonce2', 'string',
                      'hex-encoded big-endian extranonce2, length depends on extranonce2_size from mining.notify.'),
                     ('ntime', 'string',
                      'UNIX timestamp (32bit integer, big-endian, hex-encoded), must be >= ntime provided by mining,notify and <= current time'),
